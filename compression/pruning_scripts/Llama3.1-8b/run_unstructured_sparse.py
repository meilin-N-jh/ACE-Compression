#!/usr/bin/env python3
"""
无结构化稀疏剪枝脚本 - Llama3.1-8b
使用 llmcompressor 进行 50% 无结构化稀疏剪枝，使用 Alpaca 数据集校准

使用方法:
    conda activate pruning
    cd {ARTIFACT_ROOT}/pruning_scripts/Llama3.1-8b
...
import gc
# ==================== 配置 ====================
SOURCE_MODEL = f"{ARTIFACT_ROOT}/models/llama3.1-8b/Meta-Llama-3.1-8B-Instruct"
OUTPUT_MODEL = f"{ARTIFACT_ROOT}/models/llama3.1-8b/Meta-Llama-3.1-8B-Instruct-Unstructured-Sparse-50"
...
print(f"\n✓ 剪枝完成: {OUTPUT_MODEL}")
print(f"下一步: bash {ARTIFACT_ROOT}/models/llama3.1-8b/start_vllm_unstructured.sh")
