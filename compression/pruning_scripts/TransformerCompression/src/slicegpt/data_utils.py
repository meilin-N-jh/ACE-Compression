...
def get_dataset(name: str) -> datasets.DatasetDict:
    ...
    ds_properties = {
        ...
        "alpaca": {"path": "yahma/alpaca-cleaned"},
    }
    ...
    # and build text field with Instruction/Input/Output format
    if name == "alpaca":
        # 构建 Instruction/Input/Output 格式的 text 字段
        ...
        ds = ds["train"].map(build_prompt, remove_columns=ds["train"].column_names)
