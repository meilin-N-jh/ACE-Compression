...
# Run PsychoBench for Qwen2.5-14B Unstructured Sparse via vLLM OpenAI API

set -euo pipefail

PB_DIR="${ARTIFACT_ROOT}/paper_benchmarks/10_PsychoBench/benchmark"
SCRIPT="$PB_DIR/qwen2.5-14b/run_psychobench_qwen25_14b.py"
PORT="8106"
MODEL_NAME="qwen2.5-14b-unstructured-sparse"
RUN_NAME="Qwen2.5-14B-Unstructured-Sparse"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

QUESTIONNAIRES="Empathy,BFI,BSRI,EPQ-R,LMS,DTDD,ECR-R,GSE,ICB,LOT-R,EIS,WLEIS,CABIN,16P"

RESULTS_DIR="${RESULTS_DIR:-$PB_DIR/qwen2.5-14b/results}"
LOG_DIR="$PB_DIR/qwen2.5-14b/logs"
...
if ! curl -s "http://127.0.0.1:${PORT}/v1/models" > /dev/null 2>&1; then
  ...
  echo "  bash ${ARTIFACT_ROOT}/models/Qwen2.5-14B/start_vllm_unstructured_sparse.sh"
  exit 1
fi
