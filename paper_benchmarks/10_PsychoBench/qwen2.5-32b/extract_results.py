...
MODELS = ['Qwen2.5-32B-FP16', 'Qwen2.5-32B-BNB-4bit', 'Qwen2.5-32B-GPTQ-INT4', 'Qwen2.5-32B-AWQ', 'Qwen2.5-32B-2:4-Sparse', 'Qwen2.5-32B-Unstructured-Sparse']

results_dir = Path(f'{ARTIFACT_ROOT}/paper_benchmarks/10_PsychoBench/benchmark/qwen2.5-32b/results')
...
for model in MODELS:
    ...
    for md_file in md_files:
        # 从文件名提取问卷名称
        # 文件名格式: Qwen2.5-32B-FP16-BFI.md
        stem = md_file.stem
        # 移除模型前缀，找到最后一个 '-' 后的部分
        # 例如: Qwen2.5-32B-FP16-BFI -> BFI
        parts = stem.split('-')
        # 问卷名称是最后一部分
...
# 生成 CSV
output_file = Path('qwen25_32b_results_pivot.csv')
