#!/usr/bin/env python3
"""
Qwen2.5-32B vLLM API模型生成器，用于 PsychoBench 评测
...
def normalize_openai_base_url(base_url):
    """Accept either host roots or `/v1` API roots without duplicating the suffix."""
...
class Qwen25_32B_VLLMGenerator:
    def __init__(self, base_url, model_name, api_key="EMPTY", timeout=120):
        """
        初始化 Qwen2.5-32B vLLM API 生成器

        Args:
            base_url: vLLM服务器地址 (如 http://127.0.0.1:8208/v1)
            model_name: 模型名称 (如 qwen2.5-32b-fp16)
        ...
        print(f"Using Qwen2.5-32B model via vLLM API: {model_name}")
...
def get_qwen25_32b_vllm_generator(base_url, model_name, api_key="EMPTY", timeout=120, force_reload=False):
    """
    获取或创建 Qwen2.5-32B vLLM generator 实例（单例模式）
    ...
    Returns:
        Qwen25_32B_VLLMGenerator: generator实例
    """
    ...
    # 创建新的generator
    generator = Qwen25_32B_VLLMGenerator(
        base_url=base_url,
...
def qwen25_32b_vllm_generator(questionnaire, args):
    """
    ...
    # 创建generator
    generator = get_qwen25_32b_vllm_generator(
