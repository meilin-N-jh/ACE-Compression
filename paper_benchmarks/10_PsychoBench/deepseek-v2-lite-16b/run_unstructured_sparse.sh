...
# Run PsychoBench for DeepSeek-V2-Lite-16B Unstructured Sparse via vLLM OpenAI API

set -euo pipefail

PB_DIR="${ARTIFACT_ROOT}/paper_benchmarks/10_PsychoBench/benchmark"
SCRIPT="$PB_DIR/deepseek-v2-lite-16b/run_psychobench_deepseek.py"
PORT="8305"
MODEL_NAME="deepseek-v2-lite-unstructured-sparse"
RUN_NAME="DeepSeek-V2-Lite-16B-Unstructured-Sparse"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

QUESTIONNAIRES="Empathy,BFI,BSRI,EPQ-R,LMS,DTDD,ECR-R,GSE,ICB,LOT-R,EIS,WLEIS,CABIN,16P"

RESULTS_DIR="${RESULTS_DIR:-$PB_DIR/deepseek-v2-lite-16b/results}"
LOG_DIR="$PB_DIR/deepseek-v2-lite-16b/logs"
RUN_LOG="$LOG_DIR/psychobench_unstructured_sparse_${TIMESTAMP}.log"
...
if ! curl -s "http://127.0.0.1:${PORT}/v1/models" > /dev/null 2>&1; then
  ...
  echo "  bash ${ARTIFACT_ROOT}/models/DeepSeek-V2-Lite-Chat-16B/start_vllm_unstructured_sparse.sh"
  exit 1
fi
