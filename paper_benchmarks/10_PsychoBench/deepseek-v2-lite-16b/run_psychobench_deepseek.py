#!/usr/bin/env python3
"""
Run PsychoBench with DeepSeek-V2-Lite-16B via vLLM API
用法: python run_psychobench_deepseek.py --base-url http://127.0.0.1:8300/v1 --model deepseek-v2-lite-fp16 --questionnaire Empathy,BFI,BSRI --shuffle-count 1 --test-count 10
"""
...
from deepseek_vllm_generator import deepseek_vllm_generator


def main():
    parser = argparse.ArgumentParser(description="Run PsychoBench with DeepSeek-V2-Lite-16B via vLLM API")

    # 模型配置
    parser.add_argument('--base-url', required=True, type=str,
                       help='vLLM API base URL (e.g., http://127.0.0.1:8300/v1)')
    ...
    print("=" * 60)
    print("PsychoBench: DeepSeek-V2-Lite-16B via vLLM API")
    print("=" * 60)
    ...
    # 运行评测
    run_psychobench(args, deepseek_vllm_generator)
