...
# Run PsychoBench for Llama70B AWQ via vLLM OpenAI API
...
PORT="${PORT:-8001}"
MODEL_NAME="${MODEL_NAME:-llama31-70b-awq-int4}"
RUN_NAME="${RUN_NAME:-Llama-70B-AWQ}"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

QUESTIONNAIRES="${QUESTIONNAIRES:-Empathy,BFI,BSRI,EPQ-R,LMS,DTDD,ECR-R,GSE,ICB,LOT-R,EIS,WLEIS,CABIN,16P}"
...
if ! curl -s "http://127.0.0.1:${PORT}/v1/models" > /dev/null 2>&1; then
  ...
  echo "  bash ${ARTIFACT_ROOT}/models/Llama3-70b/start_vllm_awq.sh"
  exit 1
fi
