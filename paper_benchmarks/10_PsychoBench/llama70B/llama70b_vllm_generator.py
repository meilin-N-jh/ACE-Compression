#!/usr/bin/env python3
"""
Llama70B vLLM API模型生成器，用于 PsychoBench 评测
...
def convert_results(result, column_header):
    """
    优先使用官方 PsychoBench 的解析逻辑。

    官方逻辑是对每一行取最后一个字符，能避免把题号误解析成答案。
    如果模型输出格式轻微偏离，再回退到正则容错解析。
    """
    result = result.strip()
...
class Llama70B_VLLMGenerator:
    def __init__(self, base_url, model_name, api_key="EMPTY", timeout=120):
        ...
        print(f"Using Llama70B model via vLLM API: {model_name}")
...
def get_llama70b_vllm_generator(base_url, model_name, api_key="EMPTY", timeout=120, force_reload=False):
    """
    获取或创建 Llama70B vLLM generator 实例（单例模式）
    """
    ...
    # 创建新的generator
    generator = Llama70B_VLLMGenerator(
        base_url=base_url,
...
def llama70b_vllm_generator(questionnaire, args):
    """
    ...
    # 创建generator
    generator = get_llama70b_vllm_generator(
    ...
    # 如果测试文件不存在，优先复制原始模板；否则再生成。
    if not os.path.exists(args.testing_file):
        print(f"Testing file not found, preparing: {args.testing_file}")
