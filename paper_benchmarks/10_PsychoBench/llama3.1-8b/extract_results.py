...
MODELS = ['Llama3.1-8B-FP16', 'Llama3.1-8B-BNB-4bit', 'Llama3.1-8B-GPTQ-INT4', 'Llama3.1-8B-AWQ',
          'Llama3.1-8B-2:4-Sparse', 'Llama3.1-8B-Unstructured-Sparse']
...
for model in MODELS:
    ...
    for md_file in md_files:
        # 从文件名提取问卷名称
        # 文件名格式: Llama3.1-8B-FP16-BFI.md
        stem = md_file.stem
        # 移除模型前缀，找到最后一个 '-' 后的部分
        # 例如: Llama3.1-8B-FP16-BFI -> BFI
        # 需要特殊处理 16P 这种带数字的
...
# 生成 CSV
output_file = Path(f'{ARTIFACT_ROOT}/paper_benchmarks/10_PsychoBench/benchmark/llama3.1-8b/llama31_8b_results_pivot.csv')
