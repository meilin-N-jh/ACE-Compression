#!/usr/bin/env python3
"""
Run PsychoBench with Llama3.1-8B via vLLM API
用法: python run_psychobench_llama.py --base-url http://127.0.0.1:8400/v1 --model meta-llama-3.1-8b-instruct-fp16 --questionnaire Empathy,BFI,BSRI --shuffle-count 1 --test-count 10
...
from llama31_8b_vllm_generator import llama31_8b_vllm_generator


def main():
    parser = argparse.ArgumentParser(description="Run PsychoBench with Llama3.1-8B via vLLM API")

    # 模型配置
    parser.add_argument('--base-url', required=True, type=str,
                       help='vLLM API base URL (e.g., http://127.0.0.1:8400/v1)')
    parser.add_argument('--model', required=True, type=str,
                       help='Model name (e.g., meta-llama-3.1-8b-instruct-fp16)')
    ...
    print("=" * 60)
    print("PsychoBench: Llama3.1-8B via vLLM API")
    print("=" * 60)
    ...
    # 运行评测
    run_psychobench(args, llama31_8b_vllm_generator)
