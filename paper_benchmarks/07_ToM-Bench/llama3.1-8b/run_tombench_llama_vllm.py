#!/usr/bin/env python3
"""ToM-Bench evaluation for Llama3.1-8B variants using vLLM backend."""
import os
import sys
import argparse
import json
...
def default_results_dir(language: str) -> Path:
    ...
    return TOMBENCH_ROOT / "llama3.1-8b" / subdir
...
def main():
    parser = argparse.ArgumentParser(description="Run ToM-Bench evaluation with Llama3.1-8B models via vLLM")
    parser.add_argument("--model-display-name", type=str, required=True, help="Display name for results (e.g., Llama3.1-8B-FP16)")
    parser.add_argument("--base-url", type=str, required=True, help="vLLM API base URL (e.g., http://127.0.0.1:8400)")
    parser.add_argument("--model-name", type=str, required=True, help="Model name in vLLM (e.g., meta-llama-3.1-8b-instruct-fp16)")
