...
# Constants
DEFAULT_RAW_RESULTS_PATH = './raw_results.json'
DEFAULT_BENCH_RESULTS_PATH = './benchmark_results.csv'


def _get_output_dir():
	output_dir = os.environ.get('EQBENCH_OUTPUT_DIR', '.')
	os.makedirs(output_dir, exist_ok=True)
	return output_dir


def _output_file_path(file_name):
	return os.path.join(_get_output_dir(), file_name)
...
def initialize_results(run_index, benchmark_type, resume, n_iterations, run_id, model_path, lora_path, prompt_type, quantization, inference_engine, ooba_params, include_patterns, exclude_patterns, judge_params, language, eqbench_version):
	# 根据run_id动态生成结果文件路径
	raw_results_path = _output_file_path(f'raw_results_{run_id}.json')
...
def process_questions(benchmark_type, model, ooba_instance, inference_engine, results, model_path, prompt_type, tokenizer, launch_ooba, ooba_request_timeout, run_index, run_iter, verbose, n_attempts, openai_client, questions, eqbench_version, language, REVISE, judge_params, test_model_outputs, process_fn, run_id):
...
def save_and_upload_results(run_id, formatted_datetime, bench_success, prompt_type, model_path, lora_path, quantization, benchmark_type, lang_suffix, this_score, parseable, n_iterations, inference_engine, ooba_params, include_patterns, exclude_patterns, judge_params, results, run_index, last_error, bench_tries, max_bench_retries, google_spreadsheet_url, save_result_to_db_fn, eqbench_version):
	# 根据run_id动态生成结果文件路径
	bench_results_path = _output_file_path(f'benchmark_results_{run_id}.csv')
	raw_results_path = _output_file_path(f'raw_results_{run_id}.json')
...
def run_generic_benchmark(run_id, model_path, lora_path, prompt_type, quantization,
	...
	tokenizer = None
	last_error = ''
	# Ensure this is always defined for cleanup(), even if the benchmark fails early
	...
	while not bench_success and bench_tries <= max_bench_retries:
		for run_iter in range(1, n_iterations+1):			
			...
			try:
				...
				process_questions(benchmark_type, model, ooba_instance, inference_engine, results, model_path, prompt_type, tokenizer, launch_ooba, ooba_request_timeout, run_index, run_iter, verbose, n_attempts, openai_client, questions, eqbench_version, language, REVISE, judge_params, test_model_outputs, process_fn, run_id)
	...
	# 根据run_id动态生成结果文件路径（遵循 EQBENCH_OUTPUT_DIR）
	raw_results_path = _output_file_path(f'raw_results_{run_id}.json')
