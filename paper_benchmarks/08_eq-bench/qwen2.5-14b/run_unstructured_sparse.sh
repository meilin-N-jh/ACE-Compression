...
# Run EQ-Bench for Qwen2.5-14B Unstructured Sparse via vLLM OpenAI API
# 需要先手动启动vLLM服务器，并在终端激活环境: conda activate benchmark

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/qwen2.5-14b/config/config_unstructured_sparse.cfg}"
PORT="8106"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/qwen2.5-14b/results}"
RUN_LOG="$EQBENCH_DIR/qwen2.5-14b/logs/eqbench_unstructured_sparse_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/qwen2.5-14b/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Qwen2.5-14B Unstructured Sparse"
echo "=========================================="
echo "API Port: $PORT"
echo "=========================================="
echo ""
