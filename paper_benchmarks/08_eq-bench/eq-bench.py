...
def main():
	...
	parser.add_argument('--config', default='config.cfg',
						help="Path to config file. Defaults to ./config.cfg")
	...
	template_dir = os.path.join(config_dir, 'instruction-templates')
	if not os.path.exists(template_dir):
		template_dir = os.path.join(script_dir, 'instruction-templates')
	os.environ['EQBENCH_TEMPLATE_DIR'] = template_dir

	# Preprocess the configuration content
	config_content = preprocess_config_string(config_path)
	...
	# Only check ooba_launch_script if automatically_launch_ooba is True
	if launch_ooba and ooba_launch_script:
	...
	for run_id, prompt_type, model_path, lora_path, quantization, n_iterations, inference_engine, ooba_params, include_patterns, exclude_patterns in parsed_batch:
		if inference_engine == 'openai':
			template_path = os.path.join(template_dir, f"{prompt_type}.yaml")
			if not os.path.exists(template_path):
				raise Exception(
					f"OpenAI template not found: {template_path} (run_id={run_id}). "
					f"Please ensure instruction_template matches a YAML file name."
				)
