...
# Run EQ-Bench for Llama3.1-8b GPTQ-INT4 via vLLM OpenAI API
# 需要先启动vLLM服务器

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/llama3.1-8b/config/config_gptq_int4.cfg}"
PORT="8403"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/llama3.1-8b/results}"
RUN_LOG="$EQBENCH_DIR/llama3.1-8b/logs/eqbench_gptq_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/llama3.1-8b/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Llama3.1-8B GPTQ-INT4"
echo "=========================================="
...
if ! curl -s "http://127.0.0.1:${PORT}/v1/models" > /dev/null 2>&1; then
    ...
    echo "  bash ${ARTIFACT_ROOT}/models/llama3.1-8b/start_vllm_gptq.sh"
    exit 1
fi
