...
# Run EQ-Bench for Llama3.1-8B 2:4 Sparse via vLLM OpenAI API

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/llama3.1-8b/config/config_2_4_sparse.cfg}"
PORT="8410"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/llama3.1-8b/results}"
RUN_LOG="$EQBENCH_DIR/llama3.1-8b/logs/eqbench_2_4_sparse_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/llama3.1-8b/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Llama3.1-8B 2:4 Sparse"
echo "=========================================="

if ! curl -s "http://127.1:${PORT}/0.0.v1/models" > /dev/null 2>&1; then
