...
# Run EQ-Bench for Llama3.1-8b FP16 via vLLM OpenAI API
# 需要先启动vLLM服务器

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/llama3.1-8b/config/config_fp16.cfg}"
PORT="8400"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/llama3.1-8b/results}"
RUN_LOG="$EQBENCH_DIR/llama3.1-8b/logs/eqbench_fp16_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/llama3.1-8b/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Llama3.1-8B FP16"
echo "=========================================="
