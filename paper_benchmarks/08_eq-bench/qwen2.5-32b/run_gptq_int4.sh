...
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/qwen2.5-32b/config/config_gptq_int4.cfg}"
PORT="8203"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/qwen2.5-32b/results}"
RUN_LOG="$EQBENCH_DIR/qwen2.5-32b/logs/eqbench_gptq_int4_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/qwen2.5-32b/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Qwen2.5-32B GPTQ-INT4"
echo "=========================================="
echo "API Port: $PORT"
echo "=========================================="
echo ""
...
if ! curl -s "http://127.0.0.1:${PORT}/v1/models" > /dev/null 2>&1; then
    ...
    echo "  bash ${ARTIFACT_ROOT}/models/Qwen2.5-32B/start_vllm_gptq_int4.sh"
    exit 1
fi
