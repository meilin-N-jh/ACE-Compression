...
# Run EQ-Bench for Llama70B 2:4 Sparse via vLLM OpenAI API

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/llama70B/config/config_2_4_sparse.cfg}"
PORT="8010"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/llama70B/results}"
RUN_LOG="$EQBENCH_DIR/llama70B/logs/eqbench_2_4_sparse_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/llama70B/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Llama70B 2:4 Sparse"
echo "=========================================="
