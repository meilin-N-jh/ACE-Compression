...
# Run EQ-Bench for Llama70B AWQ via vLLM OpenAI API

set -euo pipefail

EQBENCH_DIR="${ARTIFACT_ROOT}/paper_benchmarks/08_eq-bench/benchmark"
CONFIG_FILE="${EQBENCH_CONFIG:-$EQBENCH_DIR/llama70B/config/config_awq.cfg}"
PORT="8000"
TIMESTAMP=$(date +%Y-%m-%d_%H%M%S)

RESULTS_DIR="${RESULTS_DIR:-$EQBENCH_DIR/llama70B/results}"
RUN_LOG="$EQBENCH_DIR/llama70B/logs/eqbench_awq_${TIMESTAMP}.log"

mkdir -p "$EQBENCH_DIR/llama70B/logs"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "EQ-Bench: Llama70B AWQ"
echo "=========================================="
